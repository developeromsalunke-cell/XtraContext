    ...
    try {
        ...
        if (name === "list_threads") {
            ...
            const text = Array.isArray(threads)
                ? threads.map((t) => `[${t._id}] ${t.title} (${t.messageCount || 0} msgs)`).join("\n")
        ...
        if (name === "get_project_summary") {
            const response = await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "get_messages", params: { threadId: args.threadId } }),
            });
            const data = await response.json();
            if (data.error)
            ...
            const text = Array.isArray(messages)
                ? messages.map((m) => `${m.role.toUpperCase()} [${m.platform || 'N/A'}] [${m.model || 'N/A'}]: ${m.content}`).join("\n\n---\n\n")
        ...
        if (name === "ask_vault") {
            const response = await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "ask_vault", params: { question: args.question } }),
            });
            const data = await response.json();
            if (!response.ok || data.error)
                throw new Error(data.error || `Proxy error: ${response.statusText}`);
        ...
        if (name === "list_todos") {
            ...
            const text = Array.isArray(todos)
                ? todos.map((t) => `${t.completed ? "[x]" : "[ ]"} ${t.task} (ID: ${t._id})`).join("\n")
        ...
        if (name === "complete_todo") {
            await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "complete_todo", params: { todoId: args.todoId } }),
            });
        ...
        if (name === "save_snapshot") {
            const response = await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "save_snapshot", params: { content: args.content } }),
            });
            const data = await response.json();
            if (!response.ok || data.error)
                throw new Error(data.error || `Proxy error: ${response.statusText}`);
        ...
        if (name === "validate_memory") {
            const response = await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "validate", params: { codeSnippet: args.codeSnippet } }),
            });
            const data = await response.json();
            if (!response.ok || data.error)
                throw new Error(data.error || `Proxy error: ${response.statusText}`);
        ...
        if (name === "synthesize_vault") {
            const response = await fetch(apiUrl, {
                method: "POST",
                headers: { "Authorization": `Bearer ${apiKey}`, "Content-Type": "application/json" },
                body: JSON.stringify({ action: "synthesize", params: { focus: args.focus } }),
            });
            const data = await response.json();
            if (!response.ok || data.error)
                throw new Error(data.error || `Proxy error: ${response.statusText}`);
