...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    const latestState = await db.contextState.findFirst({
      where: { projectId },
    ...
    const newState = await db.contextState.create({
      data: {
        projectId,
        version: nextVersion,
        content,
    ...
    const responseBody: SyncPushResponse = {
      ...
      updatedAt: newState.createdAt.toISOString(),
    };
...
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    const latestState = await db.contextState.findFirst({
      where: { projectId: parseResult.data.projectId },
      orderBy: { version: "desc" },
    });
    if (!latestState) {
    ...
    const responseBody: SyncPullResponse = {
      ...
      updatedAt: latestState.createdAt.toISOString(),
    };
